...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('kg-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('kg-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
...
KGCore.Commands.Add('setcryptoworth', 'Set crypto value', { { name = 'crypto', help = 'Name of the crypto currency' }, { name = 'Value', help = 'New value of the crypto currency' } }, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('KGCore:Notify', src, 'You have changed the value of ' .. Crypto.Labels[crypto] .. ' from: $' .. Crypto.Worth[crypto] .. ' to: $' .. NewWorth .. ' (' .. ChangeLabel .. ' ' .. PercentageChange .. '%)')
                Crypto.Worth[crypto] = NewWorth
                TriggerClientEvent('kg-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
                ...
                TriggerClientEvent('KGCore:Notify', src, Lang:t('text.you_have_not_given_a_new_value', { crypto = Crypto.Worth[crypto] }))
            end
        else
            TriggerClientEvent('KGCore:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('KGCore:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_kgit'))
    end
end, 'admin')

KGCore.Commands.Add('checkcryptoworth', '', {}, false, function(source)
    local src = source
    TriggerClientEvent('KGCore:Notify', src, Lang:t('text.the_kgit_has_a_value_of', { crypto = Crypto.Worth['kgit'] }))
end)

KGCore.Commands.Add('crypto', '', {}, false, function(source)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth['kgit'])

    TriggerClientEvent('KGCore:Notify', src, Lang:t('text.you_have_with_a_value_of', { playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto, mypocket = MyPocket }))
end)

-- Events

RegisterServerEvent('kg-crypto:server:FetchWorth', function()
    for name, _ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('kg-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('kg-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('kg-crypto:server:ExchangeFail', function()
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        exports['kg-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'kg-crypto:server:ExchangeFail')
        TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('kg-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('kg-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('kg-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('kg-crypto:server:SyncReboot', function()
    TriggerClientEvent('kg-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('kg-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName('cryptostick')
    if ItemData ~= nil then
        ...
        exports['kg-inventory']:RemoveItem(src, 'cryptostick', 1, false, 'kg-crypto:server:ExchangeSuccess')
        Player.Functions.AddMoney('crypto', Amount, 'kg-crypto:server:ExchangeSuccess')
        TriggerClientEvent('KGCore:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for', { amount = Amount }), 'success', 3500)
        TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('kg-phone:client:AddTransaction', src, Player, {}, Lang:t('credit.there_are_amount_credited', { amount = Amount }), 'Credit')
    end
end)

-- Callbacks

KGCore.Functions.CreateCallback('kg-crypto:server:HasSticky', function(source, cb)
...
KGCore.Functions.CreateCallback('kg-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = KGCore.Functions.GetPlayer(source)
    local CryptoData = {
...
KGCore.Functions.CreateCallback('kg-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = KGCore.Functions.GetPlayer(source)
    local total_price = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['kgit']))
    if Player.PlayerData.money.bank >= total_price then
        local CryptoData = {
            History = Crypto.History['kgit'],
            Worth = Crypto.Worth['kgit'],
            Portfolio = Player.PlayerData.money.crypto + tonumber(data.Coins),
            WalletId = Player.PlayerData.metadata['walletid'],
        }
        Player.Functions.RemoveMoney('bank', total_price, 'bought crypto')
        TriggerClientEvent('kg-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_kgit_purchased', { dataCoins = tonumber(data.Coins) }), 'Credit')
...
KGCore.Functions.CreateCallback('kg-crypto:server:SellCrypto', function(source, cb, data)
    local Player = KGCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        local amount = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['kgit']))
        TriggerClientEvent('kg-phone:client:AddTransaction', source, Player, data, Lang:t('debit.you_have_sold', { dataCoins = tonumber(data.Coins) }), 'Debit')
...
KGCore.Functions.CreateCallback('kg-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('kg-phone:client:AddTransaction', source, Player, data, 'You have ' .. tonumber(data.Coins) .. " Kgit('s) transferred!", 'Debit')
            local Target = KGCore.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins), 'transfer crypto')
                TriggerClientEvent('kg-phone:client:AddTransaction', Target.PlayerData.source, Player, data, 'There are ' .. tonumber(data.Coins) .. " Kgit('s) credited!", 'Credit')
            else
    ...
    Citizen.CreateThread(function()
        ...
        while (true) do
            ...
            if type(get_coin_price) == 'number' then
                Crypto.Worth['kgit'] = get_coin_price
            else
